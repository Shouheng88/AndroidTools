...
def translate(to_translate, to_language):
    ...
    try:
        ...
        response = requests.post(baidu_translate_config.TRANSLATE_URL, from_data)
...
def get_language_mapping(language):
    ...
    return baidu_translate_config.MAPPING[language]
...
def get_post_form_data(to_translate, to_language):
    ...
    dist['from'] = 'zh'
    dist['to'] = to_language
    dist['appid'] = baidu_translate_config.APP_ID
    salt = '12321321'
    dist['salt'] = salt
    sign = baidu_translate_config.APP_ID + to_translate + salt + baidu_translate_config.APP_SECRET 
    logging.debug(sign)
    hl = hashlib.md5()
